# swpu-agent

Spring Boot 3.5 + Java 17 智能 BI 对话后端，对接 Python LangChain AI Agent，实现自然语言数据查询与可视化。

## 架构

```
浏览器 / 前端
    │  SSE
    ▼
┌─────────────────────────────┐
│  Java Backend (:8080)       │
│  JWT · 会话管理 · SSE 中继    │
└──────────┬──────────────────┘
           │ RestClient
           ▼
┌─────────────────────────────┐
│  Python Agent (:8000)       │
│  LangChain · qwen3-max      │
└──────────┬──────────────────┘
           │
           ▼
┌─────────────────────────────┐
│  MySQL (agent) · Redis      │
│  共用单库，init.sql 一键建表  │
└─────────────────────────────┘
```

## 快速开始

```bash
# 1. 启动 MySQL + Redis
docker-compose up -d

# 2. 一键建库 + 建表 + 测试数据
mysql -u root -proot < sql/init.sql

# 3. 启动 Python Agent（独立项目，另见 agent-py/README.md）
cd ../agent-py && python main.py       # → :8000

# 4. 启动 Java 后端
cd ../swpu-agent && mvn spring-boot:run    # → :8080

# 5. 验证
curl http://localhost:8080/api/health
# → {"code":200,"data":{"status":"UP","version":"1.0.0"}}
```

## 环境变量

| 变量 | 默认值 | 说明 |
|------|--------|------|
| `DB_PASSWORD` | `root` | MySQL 密码 |
| `REDIS_HOST` | `localhost` | Redis 地址 |
| `REDIS_PASSWORD` | (空) | Redis 密码 |
| `JWT_SECRET_KEY` | 内置默认值 | JWT 签名密钥 |
| `AGENT_PYTHON_URL` | `http://localhost:8000` | Python Agent 地址 |

## API 端点

### 公开（无需登录）

| 方法 | 路径 | 说明 |
|------|------|------|
| `GET` | `/api/health` | 健康检查 |
| `POST` | `/api/auth/send_code` | 发送登录验证码 |
| `POST` | `/api/auth/send_register_code` | 发送注册验证码 |
| `POST` | `/api/auth/login` | 登录 → `{accessToken, refreshToken}` |
| `POST` | `/api/auth/register` | 注册 → `{accessToken, refreshToken}` |

### Chat（Bearer Token）

| 方法 | 路径 | 说明 |
|------|------|------|
| `GET` | `/api/chat/sessions` | 我的会话列表 |
| `POST` | `/api/chat/sessions` | 创建会话 |
| `GET` | `/api/chat/sessions/{id}/messages` | 消息历史 |
| `DELETE` | `/api/chat/sessions/{id}` | 删除会话 |
| `POST` | `/api/chat/send` | **发消息（SSE 流式返回）** |

### 数据库连接管理

| 方法 | 路径 | 说明 |
|------|------|------|
| `GET` | `/api/db/connections` | 连接列表 |
| `POST` | `/api/db/connections` | 新建连接 |
| `PUT` | `/api/db/connections/{id}` | 更新连接 |
| `DELETE` | `/api/db/connections/{id}` | 删除连接 |
| `POST` | `/api/db/connections/{id}/test` | 测试连接 |
| `GET` | `/api/db/connections/{id}/schema` | 查看库表结构 |

### 其他

| 方法 | 路径 | 说明 |
|------|------|------|
| `POST` | `/api/viz/generate` | 生成 ECharts 图表 |
| `GET` | `/api/user/profile` | 用户信息 |
| `PUT` | `/api/user/profile` | 更新用户信息 |

## SSE 事件

`POST /api/chat/send` 返回以下事件流：

```
event:user_saved  → {"message_id":1}
event:thinking    → 正在分析您的问题…
event:text        → 您好，请问…
event:text        → …需要查询什么数据？
event:chart       → {"option":{...}}    (图表生成时)
event:done        →                     (流结束)
event:error       → 错误信息             (异常时)
```

## 数据库

`sql/init.sql` 一键创建 `agent` 库及全部 11 张表：

```
agent
├── user_info           用户 · 角色权限
├── users               JWT 登录账号
├── chat_sessions       对话会话
├── chat_messages       对话消息
├── db_connections      外部数据库连接
├── tool_invocations    Agent 工具调用日志
├── customer            客户数据
├── products            产品数据
├── orders              订单数据
├── customer_behavior   客户行为
└── sales               销售统计
```

含一条测试用户 `admin@test.com`（角色=总经理，全权限）和示例销售数据。

## 项目结构

```
swpu-agent/
├── sql/init.sql                 一键建库脚本
├── docker-compose.yml           MySQL + Redis
├── pom.xml
└── src/main/java/com/swpuagent/
    ├── agent/AgentClient.java       调用 Python /chat（SSE 中继）
    ├── controller/
    │   ├── AuthController.java      认证（纯 Java，0 LLM）
    │   ├── ChatController.java      对话 SSE
    │   └── ...
    ├── service/
    │   ├── AgentService.java        编排：查邮箱 → 调 Python
    │   └── AuthService.java         登录/注册逻辑
    ├── security/                    JWT 过滤器
    ├── mapper/                      MyBatis 数据访问
    └── common/                      异常处理
```

## 注意

- **Auth 不走 LLM**，验证码 Java 生成存 Redis，零 AI 消耗
- Python Agent 不可用时 Chat 报错，但认证、会话管理照常工作
- 生产环境请换 `JWT_SECRET_KEY`，清理测试数据
